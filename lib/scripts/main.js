(function() {
  ...
  require(['hogan', 'jquery', 'raphael', 'io', 'underscore', 'backbone'], function(hogan) {
    var App, EE, Meter, Phone, Record, Records, Wave, socket;
    ...
    Meter = (function(_super) {
      ...
      Meter.prototype.tagName = 'section';

      Meter.prototype.className = 'meter';

      Meter.prototype.template = hogan.compile($('#meter-template').html());
        ...
        socket.on('data#v0.1', function(data) {
          ...
          if (matched.length === 0) {
            ...
            if (data.waveform) {
              ...
              _this.chart[data.name] = new Meter({
                model: (_this.collection.where({
                  name: data.name
                }))[0]
              });
